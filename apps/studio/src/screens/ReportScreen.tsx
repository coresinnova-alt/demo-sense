...
import { Badge, Button, MediaThumb } from '@sense/ui'
    ...
    <div className="animate-rise mx-auto max-w-[1000px] px-6 py-6 pb-16">
      ...
      <article className="paper rounded-xl border border-line bg-paper px-14 py-14 font-serif leading-relaxed text-paper-ink shadow-e3">
          ...
          <section key={group.no}>
            ...
            {group.subs.map((sub) => (
              <div key={sub.id} className="mb-7 break-inside-avoid">
                ...
                {/* Voice notes are field evidence, cited rather than embedded. */}
                {sub.media.some((m) => m.kind === 'audio') ? (
                  <p className="mt-2 text-[11.5px] text-ink-3 italic">
                    ...
                    {sub.media
                      ...
                      .map((m) => m.name)
                      .join(', ')}{' '}
                    — transcripts retained in the project record.
                  </p>
                ) : null}
              </div>
            ))}
        ...
        <p className="mt-9 border-t border-line pt-3 text-[11px] italic text-ink-3 text-pretty">
          ...
          computed from the seeded cost book; photos, video and voice notes auto-placed at capture. Reviewed and approved by
